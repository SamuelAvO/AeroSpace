...
private let adaptiveWeightBeforeResizeWithMouseKey = TreeNodeUserDataKey<CGFloat>(
    key: "adaptiveWeightBeforeResizeWithMouseKey")
...
private func resizeWithMouse(_ window: Window) async throws {  // todo cover with tests
    ...
    case .tilingContainer:
        ...
        let table: [(CGFloat, TilingContainer?, Int?, Int?)] = [
            ...
            (
                rect.maxY - lastAppliedLayoutRect.maxY, dParent, dOwnIndex.map { $0 + 1 },
            ...
            {  // 5 pixels should be enough to fight with accumulated floating precision error
                ...
                if parent.layout == .tiles {
                    ...
                    for sibling in parent.children[startIndex..<pastTheEndIndex] {
                        sibling.setWeight(
                            orientation, sibling.getWeightBeforeResize(orientation) - siblingDiff)
                    }
                }
            }
        }
